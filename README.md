...
깃허브에서 매일 자동으로 돌아가도록 바꾼 버전입니다.

**출석 시간 설정 방법**은 맨 아래 **`3. 실행 확인`** 를 참고 하시면 됩니다.
...
3. 디스코드 개인 서버의 웹훅 또는 타 서버의 웹훅(선택 사항)
4. 메모장 어플(선택 사항)
5. 자주 안쓰는 브라우저(선택 사항)
...
---
## 2. GitHub Secret 등록하기

1. 복사된 본인 저장소에서 → **`⚙️Settings`** 탭  → **`Secrets and variables`** → **`Actions`**
...
- **`LTUID`** = **`ltuid_v2`**   (`1. 쿠키 값 얻기`참고)
- **`LTOKEN`** = **`ltoken_v2`**   (`1. 쿠키 값 얻기`참고)
